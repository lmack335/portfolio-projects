 ...
 */
public class VSCO_Game_Lanna extends GameApplication {
...
//		super.initGame();  //automatically generated by Eclipse; do we actually need this?
		
		getGameWorld().addEntityFactory(new GoodieFactory());
...
//				.viewFromTextureWithBBox("vsco_00.png")
		...
		//instantiate goodies; spawnData includes location, itemName, collectible (image file must be itemName.png)
//		getGameWorld().spawn("Goodie", new SpawnData(500, 200).put("name", "bluescrunchy").put("collectable", true));
//		getGameWorld().spawn("Goodie", new SpawnData(600, 300).put("name", "rainbowhydroflask").put("collectable", true));
		
		//instantiate furniture; spawnData includes location, itemName, explorable (image file must be itemName.png)
		getGameWorld().spawn("Furniture", new SpawnData(325, 0).put("name", "dresser").put("explorable", true));
		getGameWorld().spawn("Furniture", new SpawnData(25, 200).put("name", "bed").put("explorable", true));
		getGameWorld().spawn("Furniture", new SpawnData(250, 0).put("name", "mirror").put("explorable", false));
	...
	//these don't seem to actually work; not sure why not...
	@Handles(eventType = "EXPLORE_FURNITURE")
	public void onExploreFurniture(VSCOGameEvent event) {
//		getGameWorld().spawn("Goodie", new SpawnData(500, 200).put("name", "bluescrunchy").put("collectable", true));
	}
	
	@Handles(eventType = "COLLECT_GOODIE")
	public void onCollectGoodie(VSCOGameEvent event) {
//		getGameState().increment("collectedObjects", +1);
//		getGameState().increment("remainingObjects", -1);
	}
	...
	protected void initUI() {
		...
		textInventory.setTranslateX(600);
		textInventory.setTranslateY(20);
		
		Text textInventoryItems = getUIFactory().newText("stuff", Color.BLACK, 18);
		textInventoryItems.setTranslateX(600);
		textInventoryItems.setTranslateY(40);
		textInventoryItems.textProperty().bind(getGameState().stringProperty("inventoryMessage"));
		
		getGameScene().addUINodes(textCollectedObjects, textRemainingObjects, 
				textActivityMessage, textInventory, textInventoryItems);
	}
